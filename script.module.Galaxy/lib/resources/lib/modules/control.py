...
# Addon Provider: The Martian, Mr.Blamo
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.Galaxy.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.Galaxy.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.Galaxy.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.Galaxy.artwork)')
...
def moderator():
    ...
    if not infoLabel('Container.PluginName') in netloc:
        infoDialog('Galaxy: Unknown screen or plugin', sound=True)
        sys.exit()

def metaFile():
    if condVisibility('System.HasAddon(script.Galaxy.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.Galaxy.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
